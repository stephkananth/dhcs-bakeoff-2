...
int trialCount = 36; //this will be set higher for the bakeoff
...
void setup() {
  ...
  rectMode(CENTER);
  textFont(createFont("Arial", inchToPix(0.3))); //sets the font to Arial that is 0.3" tall
  textAlign(CENTER);
...
void draw() {
  ...
  for (int i=0; i<trialCount; i++)
  {
    ...
    if (trialIndex==i) {
      ...
      fill(128, 60, 60, 50); //set color to semi translucent
    }
    rect(0, 0, t.z, t.z);
    ...
    //float newMX = mouseX - width/2 - t.x;
    //float newMY = mouseY - height/2 - t.y;
    
    //float rotatedMX = newMY*sin(radians(t.rotation)) + newMX*cos(radians(t.rotation));
    //float rotatedMY = newMY*cos(radians(t.rotation)) - newMX*sin(radians(t.rotation));
    
    
    
    //if ((rotatedMX - -t.z/2 <= 3) && (rotatedMY - t.z/2 <= 3)) {
    //  println("hiIIIIIIIII");
    //  fill(200);
    //  ellipse(-t.z/2, t.z/2, 10, 10);

    //}
    //else if (rotatedMX == -t.z/2 && rotatedMY == -t.z/2) {
    //  fill(200);
    //  ellipse(-t.z/2, -t.z/2, 10, 10);
  ...
  text("Trial " + (trialIndex+1) + " of " +trialCount, width/2, inchToPix(.6f), 30);
  
  if (mouseReleased == true && startTime != 0 && showButton) {
    buttonX = endCornerX;
    buttonY = endCornerY;
    fill(0,255,0);
...
void mousePressed()
{
  ...
  if (startTime == 0) //start time on the instant of the first user click
  {
    ...
    //check to see if user clicked on next button
...
void mouseDragged() {
  ...
  float d = dist(startCornerX, startCornerY, endCornerX, endCornerY);
  screenZ = constrain(d/sqrt(2), .01, inchToPix(4f)); //leave min and max alone!

  screenTransX = ((startCornerX + endCornerX) /2) - width/2;
  screenTransY = ((startCornerY + endCornerY) /2) - height/2;
  
  
  //ellipse((startCornerX + endCornerX) /2, (startCornerY + endCornerY) /2, 10, 10);
  
  
  screenRotation = degrees(atan2(endCornerY - startCornerY, endCornerX - startCornerX)) - 45;

  
  //float mouseAngle = degrees(atan2( ((startCornerY-height/2)), ((startCornerX - width/2)) ));
  //float cornerAngle = degrees(atan2( ((endCornerY-height/2)), ((endCornerX - width/2)) ));
  ...
  //float cornerAngle = degrees(atan2( ((endCornerY-height/2) - screenTransX), ((endCornerX - width/2) - screenTransY) ));
  
  //float degreesDifference = cornerAngle;
  
  //println(screenRotation);
  
  //screenRotation = 45 - degreesDifference; 
