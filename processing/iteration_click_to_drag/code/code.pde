...
float screenZ = 50f;
float leeway = screenZ / 2;


// boolean variable for mouse stick
boolean stick = false;
...
void scaffoldControlLogic()
{
    ...
    } else if(mousePressed && mouseDragged && selectedCorner == null && checkInside()) {
    ...
    if (checkInside() && !stick && mousePressed) {
       println("I am in");
       stick = true;
       stickBoxToCursor();
     }
        
    //if (mousePressed && mouseDragged && !checkInside()) {
...
boolean checkInside() {
  if (mouseX - width/2 > screenTransX - leeway  &&
      mouseX - width/2 < screenTransX + leeway  &&
      mouseY - height/2 > screenTransY - leeway &&
      mouseY - height/2 < screenTransY + leeway)
        return true;
  return false;
}
